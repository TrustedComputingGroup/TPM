...
case TPM_CC_NV_DefineSpace2:
{
    ...
    result = TPM2_NV_DefineSpace2(in);
    break;
}
...
case TPM_CC_NV_ReadPublic2:
{
    NV_ReadPublic2_In* in =
        (NV_ReadPublic2_In*)MemoryGetInBuffer(sizeof(NV_ReadPublic2_In));
    NV_ReadPublic2_Out* out =
        (NV_ReadPublic2_Out*)MemoryGetOutBuffer(sizeof(NV_ReadPublic2_Out));
    in->nvIndex = handles[0];
    if(*paramBufferSize != 0)
    {
        result = TPM_RC_SIZE;
    ...
    result = TPM2_NV_ReadPublic2(in, out);
    rSize  = sizeof(NV_ReadPublic2_Out);
    *respParmSize +=
        TPM2B_NV_PUBLIC_2_Marshal(&out->nvPublic, responseBuffer, &rSize);
    *respParmSize += TPM2B_NAME_Marshal(&out->nvName, responseBuffer, &rSize);
    break;
}
...
{
    SetCapability_In* in =
        (SetCapability_In*)MemoryGetInBuffer(sizeof(SetCapability_In));
    SetCapability_Out* out =
        (SetCapability_Out*)MemoryGetOutBuffer(sizeof(SetCapability_Out));
    in->authHandle = handles[0];
    result         = TPM2B_SET_CAPABILITY_DATA_Unmarshal(
        &in->setCapabilityData, paramBuffer, paramBufferSize);
    EXIT_IF_ERROR_PLUS(RC_SetCapability_setCapabilityData);
    if(*paramBufferSize != 0)
    {
        result = TPM_RC_SIZE;
        goto Exit;
    }
    result = TPM2_SetCapability(in);
    break;
}
