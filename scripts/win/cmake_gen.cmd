...
if [%DEBUG_CMAKE%]==[1] @echo on

if [%DO_TPMLIB%]==[] set DO_TPMLIB=0
if [%DO_PLATLIB%]==[] set DO_PLATLIB=0
if [%DO_SIMLIB%]==[] set DO_SIMLIB=0
if [%DO_TESTS%]==[] set DO_TESTS=0

set /A _TOTAL=%DO_TPMLIB% + %DO_PLATLIB% + %DO_SIMLIB% + %DO_TESTS%
if %_TOTAL% NEQ 1 (
    echo only one option may be Set
    exit /B 1
)

if NOT EXIST %cd%\scripts\win\cmake_env.cmd (
    echo expect to run script from root of tpm repo
    exit /b 1
)

call %cd%\scripts\win\cmake_env.cmd %cd%\build\parts %DEBUG_CMAKE%
if [] equ [%BUILD_ROOT%] (
...
if [%DO_TESTS%] equ [1] (
    set SOURCE_DIR=TPMCmd\tpm_tests
    set CMAKE_BUILD_SUBDIR=tpm_tests
)
